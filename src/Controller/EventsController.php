...
class EventsController extends AppController
{
	...
	{    
		if ($this->request->is('ajax')) {
            ...
            $query = "SELECT e.*, c.name as category_name, c.color as category_color, (SELECT count(l.events_id) FROM likes l WHERE l.events_id = e.id GROUP BY l.events_id) as likes_count, (SELECT l.areaname FROM address l WHERE l.events_id = e.id GROUP BY l.events_id) as areaname FROM events e LEFT JOIN categories c ON c.id = e.categories_id $joins WHERE e.active = 1 $cond order by e.date desc";
